...
describe("BaseEntity", () => {
  ...
  test("GET /base-entities/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/base-entities"}/${nonExistingId}`)
      .expect(404)
      .expect({
        statusCode: 404,
  ...
  test("GET /base-entities/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/base-entities"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
